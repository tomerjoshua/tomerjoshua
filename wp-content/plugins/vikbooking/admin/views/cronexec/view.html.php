...
class VikBookingViewCronexec extends JViewVikBooking {
	...
	function display($tpl = null) {
		...
		if ($dbo->getNumRows() == 1) {
			...
			if (file_exists(VBO_ADMIN_PATH.DIRECTORY_SEPARATOR.'cronjobs'.DIRECTORY_SEPARATOR.$cron_data['class_file'])) {
				//
				ob_start();
				require_once(VBO_ADMIN_PATH.DIRECTORY_SEPARATOR.'cronjobs'.DIRECTORY_SEPARATOR.$cron_data['class_file']);
