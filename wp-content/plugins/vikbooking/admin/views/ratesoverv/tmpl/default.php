				...
				<table class="vboverviewtable vbratesoverviewtable vbo-table" data-idroom="<?php echo $rid; ?>">
					...
					foreach ($roomrates[$rid] as $roomrate) {
						...
						<tr class="vbo-roverviewtablerow" id="vbo-roverw-<?php echo $roomrate['id'].'-'.$roomrate['idroom']; ?>">
							<td class="vbo-roverv-rplan<?php echo $rplan_minlos ? ' vbo-roverv-rplan-restricted' : ''; ?>" data-defrate="<?php echo $roomrate['cost']; ?>" data-roomname="<?php echo htmlspecialchars($roomrow['name']); ?>">
								<span class="vbo-rplan-name"><?php echo $roomrate['name']; ?></span>
							<?php
							if ($rplan_minlos) {
								?>
								<div class="vbo-roverv-rplan-restrictions">
...
function setNewRates() {
	...
	if (all_blocks !== false && toval.length > 0 && !isNaN(tovalint) && tovalint > 0.00) {
		...
		var jqxhr = jQuery.ajax({
			type: "POST",
			url: "index.php",
			data: { option: "com_vikbooking", task: "setnewrates", tmpl: "component", e4j_debug: debug_mode, id_room: vbolistener.first.idroom, id_price: vbolistener.first.rplan, rate: toval, vcm: invoke_vcm, minlos: setminlos, fromdate: vbolistener.first.toDate("yy-mm-dd"), todate: vbolistener.last.toDate("yy-mm-dd"), rateclosed: closerplan }
		}).done(function(res) {
			if (res.indexOf('e4j.error') >= 0) {
				console.log(res);
				alert(res.replace("e4j.error.", ""));
...
function modRoomRatePlan(mode) {
	...
	if (all_blocks !== false && mode.length > 0) {
		...
		var jqxhr = jQuery.ajax({
			type: "POST",
			url: "index.php",
			data: { option: "com_vikbooking", task: "modroomrateplans", tmpl: "component", e4j_debug: debug_mode, id_room: vbolistener.first.idroom, id_price: vbolistener.first.rplan, type: mode, fromdate: vbolistener.first.toDate("yy-mm-dd"), todate: vbolistener.last.toDate("yy-mm-dd") }
		}).done(function(res) {
			if (res.indexOf('e4j.error') >= 0 ) {
				console.log(res);
				alert(res.replace("e4j.error.", ""));
...
jQuery(document).ready(function() {
	...
	jQuery('#vbo-ratesoverview-calculate').click(function() {
		...
		var jqxhr = jQuery.ajax({
			type: "POST",
			url: "index.php",
			data: { option: "com_vikbooking", task: "calc_rates", tmpl: "component", id_room: idroom, checkin: checkindate, num_nights: nights, num_adults: adults, num_children: children }
		}).done(function(res) {
			res = JSON.parse(res);
			res = res[0];
			if (res.indexOf('e4j.error') >= 0 ) {
