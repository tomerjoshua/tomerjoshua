...
if (empty($rows)) {
	...
	jQuery(document).ready(function() {
		var jqxhr = jQuery.ajax({
			type: "POST",
			url: "index.php",
			data: { option: "com_vikbooking", task: "getgeoinfo", tmpl: "component", ips: <?php echo json_encode($missing_ips); ?> }
		}).done(function(res) {
	...
	<div class="vbo-trackings-outer-response">
		...
		foreach ($this->stats_data as $stat) {
			...
			$in_info = getdate(strtotime($stat['checkin']));
			$out_dt  = date('Y-m-d', strtotime($stat['checkout']));
		...
		$avg_conv_rate = 100 * count($totbookings) / count($totidentifiers);
		...
		$avg_los = array_sum($los_pool) / count($los_pool);
