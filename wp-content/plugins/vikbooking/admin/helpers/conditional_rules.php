...
class VikBookingHelperConditionalRules
{
	...
	public function parseTokens(&$tmpl)
	{
		...
		foreach ($matches[0] as $token) {
			...
			// all rules were compliant, apply message and trigger callback actions
			...
			foreach ($cond_texts[$token]['rules'] as $rule_data) {
				...
				// inject params and booking to rule, then trigger callback action
				$rule->setParams($rule_data->params)->setProperties($prop_keys, $prop_vals)->callbackAction();
			}
		}
