...
<div class="vbstepsbarcont">
	...
	if ($is_package === true) {
		...
		<li class="vbo-step vbo-step-complete"><a href="<?php echo JRoute::rewrite('index.php?option=com_vikbooking&task=showprc&checkin='.$first.'&checkout='.$second.'&roomsnum='.$roomsnum.'&days='.$days.$peopleforlink.$roomoptforlink.$roomindexlink.(!empty($pitemid) ? '&Itemid='.$pitemid : ''), false); ?>"><?php echo JText::translate('VBSTEPOPTIONS'); ?></a></li>
		<?php
	}
	?>
	...
	if (is_array($coupon)) {
		...
		if ($coupontotok) {
			...
			if ($coupon['percentot'] == 1) {
				 ...
				 */
				$couponsave = ($coupon['excludetaxes'] ? ($totdue - $tot_taxes - $tot_city_taxes - $tot_fees) : $totdue) * $coupon['value'] / 100;
				$totdue = ($coupon['excludetaxes'] ? ($totdue - $tot_taxes - $tot_city_taxes - $tot_fees) : $totdue) * $minuscoupon / 100;
	...
	?>
		...
		if ($usedcoupon == true) {
			...
			<div class="vbo-oconfirm-summary-room-row vbo-oconfirm-summary-total-row vbo-oconfirm-summary-coupon-row">
				<div class="vbo-oconfirm-summary-room-cell-descr">
					<span><?php echo JText::translate('VBCOUPON'); ?> <?php echo $coupon['code']; ?></span>
				</div>
				<div class="vbo-oconfirm-summary-room-cell-guests">
					<span></span>
				</div>
	...
	foreach ($cfields as $cf) {
		...
		if ($cf['type'] == "text") {
			...
			<div class="vbo-oconfirm-cfield-entry">
				...
				<?php
				if ($cf['isphone'] == 1) {
					echo $vbo_app->printPhoneInputField(array('name' => 'vbf' . $cf['id'], 'id' => 'vbf-inp' . $cf['id'], 'value' => $def_textval, 'class' => 'vbinput', 'size' => '40'));
				} else {
			...
			foreach ($countries as $country) {
				$countries_sel .= '<option data-threecode="' . $country['country_3_code'] . '" value="' . $country['country_3_code'] . '::' . $country['country_name'] . '"' . ($country['country_3_code'] == $usercountry || $cf['defvalue'] == $country['country_3_code'] ? ' selected="selected"' : '') . '>' . $country['country_name'] . '</option>' . "\n";
			}
	...
	foreach ($payments as $pk => $pay) {
		...
		if (!empty($pay['logo'])) {
			...
			?>
				<span class="vbo-payment-image">
					<label for="gpay<?php echo $pay['id']; ?>">
						<img src="<?php echo $logo['uri']; ?>" alt="<?php echo $pay['name']; ?>"/>
					</label>
				</span>
			<?php
		}
