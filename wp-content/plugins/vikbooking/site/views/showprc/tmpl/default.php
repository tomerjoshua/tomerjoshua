...
<div class="vbo-showprc-head-wrapper">
	...
	foreach ($this->rooms as $num => $r) {
		...
		if (!empty($r['idopt'])) {
			...
			VikBooking::filterOptionalsByDate($optionals, $this->checkin, $this->checkout);
		...
		<div class="room_container">
			...
			if ($this->roomsnum > 1) {
				?>
				<div class="vbshowprcroomnum"><?php echo JText::translate('VBSEARCHROOMNUM'); ?> <?php echo $num; ?></div>
				<?php
			}
			?>
				...
				<div class="vbroomimgdesc">
					<div class="vikfx-gallery-container vikfx-showprc-gallery-container">
						<div class="vikfx-gallery-fade-container">
							<img alt="<?php echo $r['name']; ?>" src="<?php echo VBO_SITE_URI; ?>resources/uploads/<?php echo $r['img']; ?>" class="vikfx-gallery-image vblistimg"/>
						<?php
					...
					foreach ($gallery_data[$num] as $mimg) {
						?>
							<a href="<?php echo $mimg['big']; ?>">
								<img src="<?php echo $mimg['thumb']; ?>" alt="<?php echo $mimg['alt']; ?>" title="<?php echo $mimg['caption']; ?>"/>
							</a>
						<?php
					}
					?>
	...
	foreach ($this->arrpeople as $indroom => $aduch) {
		...
		<input type="hidden" name="days" value="<?php echo $this->tars[1][0]['days']; ?>"/>
