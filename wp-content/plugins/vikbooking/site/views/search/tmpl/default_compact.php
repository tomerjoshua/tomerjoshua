				...
				<div class="vbo-room-result-body-img">
					<div class="vikfx-gallery-container vikfx-compact-gallery-container">
						<div class="vikfx-gallery-fade-container">
							<img class="vblistimg" alt="<?php echo $this->escape($roominfo['name']); ?>" id="vbroomimg<?php echo $idroom; ?>" src="<?php echo VBO_SITE_URI; ?>resources/uploads/<?php echo $roominfo['img']; ?>"/>
						<?php
...
function vboCompactSelRoom(indroom, idroom, units) {
	...
	if (adding_units) {
		...
		for (var i = (removing_units - 1); i >= 0; i--) {
			// iterate the pool of rooms assigned starting from the last one added
			if (!vboRoomsAssigned[idroom][indroom].hasOwnProperty(i)) {
				continue;
			}
			// current-last index added
			var rmparty = vboRoomsAssigned[idroom][indroom][i];
			// unset room booked
			document.getElementById('roomopt' + rmparty).value = '';
			vboRoomsAssigned[idroom][indroom].splice(i, 1);
