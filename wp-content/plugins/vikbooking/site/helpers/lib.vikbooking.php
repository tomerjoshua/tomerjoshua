...
class VikBooking
{
	...
	public static function getDateSeparator($skipsession = true)
	{
		...
		if ($skipsession) {
			...
			$dbo->execute();
			$s = $dbo->loadAssocList();
			$getDateSeparator = empty($s[0]['setting']) ? "/" : $s[0]['setting'];
			return $getDateSeparator;
		}
	 ...
	 */
	public static function loadBusyRecords($roomids = array(), $from_ts = 0, $max_ts = 0)
	{
	...
	public static function loadOrdersRoomsData($idorder) {
		...
		$dbo->setQuery($q);
		$dbo->execute();
		return $dbo->getNumRows() ? $dbo->loadAssocList() : array();
	}
	...
	public static function sayOptionalsPlusIva($cost, $idiva, $force = false) {
		...
		$dbo = JFactory::getDbo();
		$ivainclusa = self::ivaInclusa();
		if (intval($ivainclusa) == 0 || $force === true) {
	...
	public static function getCountriesArray($tn = true)
	{
		...
		$q = "SELECT `id`, `country_name`, `country_3_code` FROM `#__vikbooking_countries` ORDER BY `country_name` ASC;";
		$dbo->setQuery($q);
		$dbo->execute();
		if (!$dbo->getNumRows()) {
			return array();
	 ...
	 * 
	 * @param 	string 	$pfile 	 the name of the cron job driver file.
	 * @param 	string 	$pparams the JSON encoded string of current params.
	 *  
	...
	public static function displayCronParameters($pfile, $pparams = '')
	{
		...
		if (!is_file(VBO_ADMIN_PATH . DIRECTORY_SEPARATOR . 'cronjobs' . DIRECTORY_SEPARATOR . $pfile)) {
			return $html;
		}

		// include driver
		require_once VBO_ADMIN_PATH . DIRECTORY_SEPARATOR . 'cronjobs' . DIRECTORY_SEPARATOR . $pfile;

		if (!class_exists('VikCronJob') || !method_exists('VikCronJob', 'getAdminParameters')) {
			return $html;
		}
		...
		if (!is_array($pconfig) || !count($pconfig)) {
			return $html;
		}
		...
		foreach ($pconfig as $value => $cont) {
			...
			switch ($cont['type']) {
				...
				case 'password':
					$html .= '<div class="btn-wrapper input-append">';
					$html .= '<input type="password" name="vikcronparams['.$value.']" value="'.(isset($arrparams[$value]) ? JHtml::fetch('esc_attr', $arrparams[$value]) : JHtml::fetch('esc_attr', $default_paramv)).'" size="20"' . $inp_attr . '/>';
				...
				case 'number':
					...
					$html .= '<input type="number" name="vikcronparams['.$value.']" value="'.(isset($arrparams[$value]) ? JHtml::fetch('esc_attr', $arrparams[$value]) : JHtml::fetch('esc_attr', $default_paramv)).'" ' . implode(' ', $number_attr) . $inp_attr . '/>';
					break;
				case 'textarea':
					$html .= '<textarea name="vikcronparams['.$value.']"' . $inp_attr . '>'.(isset($arrparams[$value]) ? JHtml::fetch('esc_textarea', $arrparams[$value]) : JHtml::fetch('esc_textarea', $default_paramv)).'</textarea>';
					break;
				case 'visual_html':
					$tarea_cont = isset($arrparams[$value]) ? JHtml::fetch('esc_textarea', $arrparams[$value]) : JHtml::fetch('esc_textarea', $default_paramv);
					...
					break;
				case 'hidden':
					$html .= '<input type="hidden" name="vikcronparams['.$value.']" value="'.(isset($arrparams[$value]) ? JHtml::fetch('esc_attr', $arrparams[$value]) : JHtml::fetch('esc_attr', $default_paramv)).'"' . $inp_attr . '/>';
					break;
				case 'checkbox':
					// always display a hidden input value turned off before the actual checkbox to support the "off" (0) status
					$html .= '<input type="hidden" name="vikcronparams['.$value.']" value="0" />';
					$html .= self::getVboApplication()->printYesNoButtons('vikcronparams['.$value.']', JText::translate('VBYES'), JText::translate('VBNO'), (isset($arrparams[$value]) ? (int)$arrparams[$value] : (int)$default_paramv), 1, 0);
					break;
				default:
					$html .= '<input type="text" name="vikcronparams['.$value.']" value="'.(isset($arrparams[$value]) ? JHtml::fetch('esc_attr', $arrparams[$value]) : JHtml::fetch('esc_attr', $default_paramv)).'" size="20"' . $inp_attr . '/>';
					break;
			}
			if (strlen($labelhelp) > 0) {
				$html .= '<span class="vbo-param-setting-comment">'.$labelhelp.'</span>';
	...
	public static function getVCMSubscriptionStatus()
	{
		...
		try {
			if (!class_exists('VikChannelManager')) {
				require_once $vcm_lib_path;
				require_once VCM_SITE_PATH . DIRECTORY_SEPARATOR . "helpers" . DIRECTORY_SEPARATOR . "vcm_config.php";
			}
			if (!method_exists('VikChannelManager', 'checkSubscriptionReminder')) {
				// VCM is outdated
				return null;
			}
			...
			if ($is_admin) {
				...
				if (VikChannelManager::isSubscriptionExpired()) {
					...
					if (VikChannelManager::checkUpdates()) {
						...
						JFactory::getApplication()->redirect('index.php?option=com_vikchannelmanager&task=update_program&forcecheck=1');
						exit;
					}
				}
