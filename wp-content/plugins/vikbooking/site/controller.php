...
class VikBookingController extends JControllerVikBooking
{
			...
			if (is_array($cfields)) {
				...
				foreach ($cfields as $cf) {
					...
					if($cf['type'] != 'separator' && $cf['type'] != 'country' && ( $cf['type'] != 'checkbox' || ($cf['type'] == 'checkbox' && intval($cf['required']) != 1) ) ) {
						$arrcustdata[JText::translate($cf['name'])] = VikRequest::getString('vbf' . $cf['id'], '', 'request');
			...
			if ($suffdata === true) {
				...
				if (VikBooking::dayValidTs($pdays, $pcheckin, $pcheckout)) {
					...
					if (count($rooms) != $proomsnum) {
						...
						$mainframe->redirect(JRoute::rewrite('index.php?option=com_vikbooking'));
						exit;
					}
									...
									if ($couponroomok) {
										...
										if ($coupontotok) {
											...
											if ($coupon['percentot'] == 1) {
												 ...
												 * 
												 * @since 	1.3.5
												 * @since 	1.4.3 we also exclude the amount of taxes beside the mandatory fees.
												 */
												$coupondiscount = ($coupon['excludetaxes'] ? ($isdue - $tot_taxes - $tot_city_taxes - $tot_fees) : $isdue) * $coupon['value'] / 100;
												$isdue = ($coupon['excludetaxes'] ? ($isdue - $tot_taxes - $tot_city_taxes - $tot_fees) : $isdue) * $minuscoupon / 100;
												$tot_taxes = $coupon['excludetaxes'] ? $tot_taxes : ($tot_taxes * $minuscoupon / 100);
							...
							if ($checkts <= $pcheckin && $checkts < $pcheckout && $pcheckin < $pcheckout) {
								...
								if ($roomsavailable === true) {
									...
									if ($must_payment && !is_array($payment)) {
										...
										$mainframe->redirect(JRoute::rewrite("index.php?option=com_vikbooking&task=oconfirm".$strpriceid.$stroptid.$strroomid.$straduchild."&roomsnum=".$proomsnum."&days=".$pdays."&checkin=".$pcheckin."&checkout=".$pcheckout.(!empty($pitemid) ? "&Itemid=".$pitemid : ""), false));
										exit;
									}
									...
									if (!(count($mod_booking) > 0) && ((is_array($payment) && intval($payment['setconfirmed']) == 1) || !$must_payment || ($usedcoupon && $isdue <= 0))) {
												...
												if (count($proomindex) == count($rooms) && !empty($proomindex[($num - 1)])) {
													...
													if ($force_rindex) {
														...
														if (!isset($room_indexes_forcemap[$r['id']])) {
															$room_indexes_forcemap[$r['id']] = array();
														}
										...
										VikBooking::sendBookingEmail($neworderid, array('guest', 'admin'));
										//
										
										// VikBooking::sendAdminMail($admail.';_;'.$useremail, JText::sprintf('VBNEWORDER', $neworderid), $ftitle, $nowts, $custdata, $rooms, $pcheckin, $pcheckout, $pricestr, $optstr, $isdue, JText::translate('VBCOMPLETED'), $payment['name'], $strcouponeff, $arrpeople, $confirmnumber);
										// VikBooking::sendCustMail($useremail, strip_tags($ftitle) . " " . JText::translate('VBORDNOL'), $ftitle, $nowts, $custdata, $rooms, $pcheckin, $pcheckout, $pricestr, $optstr, $isdue, $viklink, JText::translate('VBCOMPLETED'), $neworderid, $strcouponeff, $arrpeople, $confirmnumber);
										...
										$mainframe->redirect(JRoute::rewrite("index.php?option=com_vikbooking&view=booking&sid=" . $sid . "&ts=" . $nowts . (!empty($pnodep) ? "&nodep=".$pnodep : "") . (!empty($pitemid) ? "&Itemid=" . $pitemid : ""), false));
										...
										//
										// VikBooking::sendAdminMail($admail.';_;'.$useremail, JText::sprintf('VBOMODDEDORDER', $mod_booking['id']), $ftitle, $nowts, $custdata, $rooms, $pcheckin, $pcheckout, $pricestr, $optstr, $isdue, JText::translate('VBCOMPLETED'), "", $strcouponeff, $arrpeople, $mod_booking['confirmnumber']);
										// VikBooking::sendCustMail($useremail, strip_tags($ftitle)." ".JText::translate('VBOMODDEDORDERC'), $ftitle, $nowts, $custdata, $rooms, $pcheckin, $pcheckout, $pricestr, $optstr, $isdue, $viklink, JText::translate('VBCOMPLETED'), $mod_booking['id'], $strcouponeff, $arrpeople, $mod_booking['confirmnumber']);
										//SMS
										...
										$mainframe->enqueueMessage(JText::translate('VBOBOOKINGMODOK'));
										$mainframe->redirect(JRoute::rewrite("index.php?option=com_vikbooking&view=booking&sid=" . $sid . "&ts=" . $nowts . (!empty($pnodep) ? "&nodep=".$pnodep : "") . (!empty($pitemid) ? "&Itemid=" . $pitemid : ""), false));
									} else {
										...
										// VikBooking::sendAdminMail($admail.';_;'.$useremail, JText::sprintf('VBNEWORDER', $neworderid), $ftitle, $nowts, $custdata, $rooms, $pcheckin, $pcheckout, $pricestr, $optstr, $isdue, JText::translate('VBINATTESA'), $payment['name'], $strcouponeff, $arrpeople);
										// VikBooking::sendCustMail($useremail, strip_tags($ftitle) . " " . JText::translate('VBORDNOL'), $ftitle, $nowts, $custdata, $rooms, $pcheckin, $pcheckout, $pricestr, $optstr, $isdue, $viklink, JText::translate('VBINATTESA'), $neworderid, $strcouponeff, $arrpeople);
			...
			if ($dbo->getNumRows() > 0) {
				...
				if (!empty($rows[0]['lang'])) {
					...
					if($lang->getTag() != $rows[0]['lang']) {
						$lang->load('com_vikbooking', VIKBOOKING_SITE_LANG, $rows[0]['lang'], true);
					}
				}
				//
				$vbo_tn = VikBooking::getTranslator();
				if ($rows[0]['status'] != 'confirmed' || (VikBooking::multiplePayments() && $rows[0]['paymcount'] > 0)) {
					...
					$ftitle = VikBooking::getFrontTitle();
					$nowts = time();
					// @wponly 	we do not need to pass the "best item id" or use externalroute as it will be done later by the lib
					$viklink = JUri::root() . "index.php?option=com_vikbooking&view=booking&sid=" . $psid . "&ts=" . $pts;
					$rooms = array();
					$tars = array();
					$arrpeople = array();
					...
					$return_url = JUri::root() . "index.php?option=com_vikbooking&view=booking&sid=" . (!empty($rows[0]['idorderota']) && !empty($rows[0]['channel']) ? $rows[0]['idorderota'] : $rows[0]['sid']) . "&ts=" . $rows[0]['ts'];
					$error_url = JUri::root() . "index.php?option=com_vikbooking&view=booking&sid=" . (!empty($rows[0]['idorderota']) && !empty($rows[0]['channel']) ? $rows[0]['idorderota'] : $rows[0]['sid']) . "&ts=" . $rows[0]['ts'];
					$notify_url = JUri::root() . "index.php?option=com_vikbooking&task=notifypayment&sid=" . (!empty($rows[0]['idorderota']) && !empty($rows[0]['channel']) ? $rows[0]['idorderota'] : $rows[0]['sid']) . "&ts=" . $rows[0]['ts']."&tmpl=component";
					$model 	= JModel::getInstance('vikbooking', 'shortcodes', 'admin');
					$itemid = $model->best(array('booking'), (!empty($rows[0]['lang']) ? $rows[0]['lang'] : null));
					$extra_data = array();
					if ($itemid) {
					...
					$obj = JPaymentDispatcher::getInstance('vikbooking', $payment['file'], array_merge($rows[0], $extra_data), $payment['params']);
					...
					if (!empty($rows[0]['idorderota']) && !empty($rows[0]['channel']) && !empty($rows[0]['paymentlog'])) {
						if (stripos($rows[0]['paymentlog'], 'card number') !== false && strpos($rows[0]['paymentlog'], '*') !== false) {
							$newpaymentlog = $rows[0]['paymentlog'] . "\n----------\n" . date('c') . "\n" . $array_result['log'];
						}
					}
					//
					...
					if ($array_result['verified'] == 1) {
						...
						if (isset($array_result['tot_paid'])) {
							...
							if ($shouldpay != $totreceived && $shouldpay_befdep != $totreceived && $rows[0]['paymcount'] == 0) {
								...
								$vbo_app = VikBooking::getVboApplication();
								$adsendermail = VikBooking::getSenderMail();
								$vbo_app->sendMail($adsendermail, $adsendermail, $recipient_mail, $adsendermail, JText::translate('VBTOTPAYMENTINVALID'), JText::sprintf('VBTOTPAYMENTINVALIDTXT', $rows[0]['id'], $totreceived." (".$array_result['tot_paid'].")", $shouldpay), false);
							}
							/**
						...
						//
						$q = "UPDATE `#__vikbooking_orders` SET `status`='confirmed'" . (isset($array_result['tot_paid']) && $array_result['tot_paid'] ? ", `totpaid`='" . ($array_result['tot_paid'] + $rows[0]['totpaid']) . "', `paymcount`=".($rows[0]['paymcount'] + 1) : "") . (!empty($array_result['log']) ? ", `paymentlog`=".$dbo->quote($newpaymentlog) : "") . ", `payable`=" . $dbo->quote($new_payable) . " WHERE `id`='" . $rows[0]['id'] . "';";
						$dbo->setQuery($q);
						$dbo->execute();
						//Assign room specific unit
						...
						if ($set_room_indexes === true) {
							$q = "SELECT `id`,`idroom`,`roomindex` FROM `#__vikbooking_ordersrooms` WHERE `idorder`=".(int)$rows[0]['id'].";";
							$dbo->setQuery($q);
							$dbo->execute();
						...
						// check if some of the rooms booked have shared calendars
						VikBooking::updateSharedCalendars($rows[0]['id'], array(), $rows[0]['checkin'], $rows[0]['checkout']);
						//

						// send email notification to guest and admin
						VikBooking::sendBookingEmail($rows[0]['id'], array('guest', 'admin'));
						//
						
						// VikBooking::sendAdminMail($admail.';_;'.$rows[0]['custmail'], JText::sprintf('VBORDERPAYMENT', $rows[0]['id']), $ftitle, $nowts, $rows[0]['custdata'], $rooms, $rows[0]['checkin'], $rows[0]['checkout'], $pricestr, $optstr, $isdue, JText::translate('VBCOMPLETED'), $payment['name'], $rows[0]['coupon'], $arrpeople, $confirmnumber);
						// VikBooking::sendCustMail($rows[0]['custmail'], strip_tags($ftitle) . " " . JText::translate('VBRENTALORD'), $ftitle, $nowts, $rows[0]['custdata'], $rooms, $rows[0]['checkin'], $rows[0]['checkout'], $pricestr, $optstr, $isdue, $viklink, JText::translate('VBCOMPLETED'), $rows[0]['id'], $rows[0]['coupon'], $arrpeople, $confirmnumber);
						...
						//Booking History
						VikBooking::getBookingHistoryInstance()->setBid($rows[0]['id'])->setExtraData($tn_data)->store('P'.($rows[0]['paymcount'] > 0 ? 'N' : '0'), $payment['name']);
						//
						//invoke VikChannelManager
						if (file_exists(VCM_SITE_PATH . DS . "helpers" . DS . "synch.vikbooking.php")) {
							require_once(VCM_SITE_PATH . DS . "helpers" . DS . "synch.vikbooking.php");
				...
				if (!empty($use_otas_rmod)) {
					if ($use_otas_rmod == '+') {
						...
						if ($use_otas_rmodpcent > 0) {
							...
							// absolute
							$ch_final_cost += $use_otas_rmodval;
						}
					} else {
	...
	public function upsellextras()
	{
		...
		foreach ($extras_booked as $extra) {
			...
			$optcost = intval($o['perday']) == 1 ? ($o['cost'] * $order['days']) : $o['cost'];
		...
		/**
		 * Important: the 'paymcount' should be increased only if the status is "confirmed"
		 * or no rooms will be occupied when receiving a payment.
		 */
	...
	public function get_avcalendars_data()
	{
		...
		if (empty($rid) || empty($direction) || empty($fromdt) || empty($nextdt) || empty($prevdt)) {
			throw new Exception("Invalid request variables", 500);
		}
		...
		if (!$start_ts || !$start_info) {
			throw new Exception("Invalid date provided", 500);
		}
		// backward navigation
		if ($direction == 'prev') {
			...
			$start_info = getdate($start_ts);
			if (!$start_ts || !$start_info) {
				throw new Exception("Invalid date calculated", 500);
			}
		}
		...
		if ($start_ts < $min_lim_ts) {
			throw new Exception("Dates in the past not allowed", 500);
		}
		...
		if (!$nextnav_ts || !$nextnav_info) {
			throw new Exception("Invalid next navigation date provided", 500);
		}
		$prevnav_ts = strtotime($prevdt);
		$prevnav_info = getdate($prevnav_ts);
		if (!$prevnav_ts || !$prevnav_info) {
			throw new Exception("Invalid prev navigation date provided", 500);
		}
		...
		if (!empty($max_date_future)) {
			...
			if ($quantlim == 'm' || $quantlim == 'y') {
				...
				if ($start_ts > $max_endts_future) {
					throw new Exception("Maximum date in the future exceeded", 500);
				}
			}
		}
